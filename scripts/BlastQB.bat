...
:: Usage: 
:: 		Installer_Generator.bat <version>

:: EXEVERSION must be in x.x.x.x format for NSIS to be happy
set EXEVERSION=%1
set IMAGENAME=BlastQB-%EXEVERSION%.image
set CHANGESNAME=BlastQB-%EXEVERSION%.changes
set MYEXECNAME=BlastQB.exe
set NSINAME=InstallerScript.nsi
...
:: Set output log file
set OUT="%NEWDIR%test.nsi.output"
...
:64BIT
	echo 64-bit...
	:: START /wait cmd.exe /c "pushd %programfiles(x86)%GnuWin32bin & wget" --directory-prefix=%CD% --no-check-certificate %INSTALLER_RESOURCE_URL%
	:: START /wait cmd.exe /c "pushd %programfiles(x86)%GnuWin32bin & wget" --directory-prefix=%CD% 
	:: popd
	...
	START cmd.exe /c  "cd %programfiles(x86)%NSISUnicode & makensis /DVersion="%EXEVERSION%" /V4 %SCRIPTNAME% > %OUT%"
	goto CLEAN

:32BIT
	echo 32-bit...
	:: START /wait cmd.exe /c "pushd %programfiles%GnuWin32bin & wget" --directory-prefix=%CD% --no-check-certificate %INSTALLER_RESOURCE_URL%
	:: START /wait cmd.exe /c "pushd %programfiles%GnuWin32bin & wget" --directory-prefix=%CD%
	:: popd
	:: Uncompress resource files
	...
	:: popd
	:: Call the MakeNSIS command
	START cmd.exe /c "cd %programfiles%NSISUnicode & makensis /DVersion="%EXEVERSION%" /V4 %SCRIPTNAME% > %OUT%"
	GOTO CLEAN
